...
const genStockRow = () => {
  ...
  // return `'${symbol}', '${name}', ${owners}, ${analystHold}`;
  // return [symbol, name, owners, analystHold];
...
const genCSV = async (filename, genRow, rowCount) => {
  const csvFile = path.resolve(__dirname, 'seedFiles', filename);
  const encoding = 'utf-8';
  const writer = fs.createWriteStream(csvFile);
  let i = rowCount;

  const write = (cb, rowIds = []) => {
    let canWrite = true;
    do {
      i -= 1;
      const rowData = genRow();
      rowIds.push(rowData[0]);

      if (i % 50000 === 0) {
        fancy(`${rowCount - i}/${rowCount} rows written for ${filename}`);
      }

      if (i === 0) {
        writer.write(rowData[1], encoding, () => cb(rowIds));
      } else {
        canWrite = writer.write(rowData[1], encoding);
      }
    } while (i > 0 && canWrite);

    if (i > 0) {
      writer.once('drain', () => write(cb, rowIds));
    }
  };
